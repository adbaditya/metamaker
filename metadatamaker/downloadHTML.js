...
function downloadHTML(record,institution_info) {
	...
	if (checkExists(record.image_url)) {
        displayTags += '\t\t\t<dt>Cover Image:</dt>\n';
        displayTags += '\t\t\t<dd><img itemprop="image" src="' + record.image_url + '" alt="Book cover" style="max-width: 300px;"/></dd>\n';
    }
	...
	if (checkExists(record.isbn)) {
		displayTags += buildTag('isbn',record.isbn,false,'Serial No');
	}
